...
const createMenuHierarchy = (menuData) => {
  const menuName = park + "-footer-nav"
  let tree = [],
     mappedArr = {},
     arrElem,
     mappedElem
...
const buildMenu = menuArray => {
  ...
  for(let item in menuArray) {
    if(menuArray[item].children.length !== 0) {
      menu.push(
        <ul className="footer__nav-list" key={menuArray[item].drupal_id}>
          <li className="footer__nav-list-item footer__nav-list-item--heading">{buildLink(menuArray[item])}</li>
          {buildMenu(menuArray[item].children)}
        </ul>)
    } else {
      menu.push(<li className="footer__nav-list-item" key={menuArray[item].drupal_id}>{buildLink(menuArray[item])}</li>)
    }
  }
