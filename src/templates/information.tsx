...
export default function InformationTemplate({ data }) { 
  ...
  const fieldInfoPulloutColumns = data?.nodeInformation?.relationships?.field_info_pullout?.relationships?.field_info_pullout_columns?(data?.nodeInformation?.relationships?.field_info_pullout?.relationships?.field_info_pullout_columns):(null)
  const infoPulloutColumns = fieldInfoPulloutColumns!=null? (fieldInfoPulloutColumns.map(
  ...
  let embedMap 
  const includeMap = data?.nodeInformation?.relationships?.field_info_pullout?.field_include_tiles_map?(data?.nodeInformation?.relationships?.field_info_pullout?.field_include_tiles_map):(false)
  if (includeMap == true) {
  ...
  const embedMapSection = embedMap?(
  ...
  const pulloutAndEmbedMapSection = infoPulloutColumns!=null? (
      <section className='pb-xlarge'>
